...
Object.defineProperty(apiLoader.services['ssm'], '2014-11-06', {
  get: function get() {
    var model = __nccwpck_require__(4596);
    model.paginators = (__nccwpck_require__(5135)/* .pagination */ .o);
    model.waiters = (__nccwpck_require__(8523)/* .waiters */ .V);
    return model;
  },
  enumerable: true,
  configurable: true
...
async function run() {
    try {
        ...
        if (environmentVariables) {
            const parsedEnvironmentVariables = JSON.parse(environmentVariables);
            for (const key in parsedEnvironmentVariables) {
                const secretName = prefix + key;
                console.log(`putting ${secretName} into SSM`);
        ...
        if (secrets) {
            ...
            for (const key in parsedSecrets) {
                ...
                const secretName = prefix + key;
                console.log(`putting secret ${secretName} into SSM`);
